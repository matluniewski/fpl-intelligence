...
The package parameterizes versioned FPL rules rather than claiming that test values describe a current season. It does not implement team import adapters, provider access, persistence, optimization, news processing, authentication, account actions, or UI.
...
FPL-19 adds `@fpl-intelligence/projection` with the deterministic expected-points baseline. It consumes versioned scoring rules, explicitly weighted and traceable per-90 signals, fixture-specific minutes and probabilities, an explicit evaluation time, and a freshness policy. It returns per-player/gameweek expected points with component formulas, fixture breakdowns, assumptions, timestamps, rules/model versions, warnings, and provenance references. Invalid or stale inputs fail with typed issues; blank and double gameweeks remain explicit. The baseline excludes news adjustments and uses only project-authored synthetic rules and values in tests. No live scoring-rule claim, provider, persistence model, optimizer, or UI behavior is selected.

FPL-21 extends `@fpl-intelligence/domain` with distinct provider-independent `RawNewsItem`, `Claim`, `Evidence`, `NewsSignal`, and `PlayerAvailabilityState` contracts. Untrusted extraction candidates are runtime-validated before becoming unresolved Claims; model-assisted lineage is versioned; contradictory Evidence can coexist; unknown permissions are represented as policy-blocked; and projection-facing availability retains signal, evidence, provenance, freshness, confidence, conflict, and rule lineage. Raw content remains outside the durable contract. The implementation selects no live source, provider, LLM, persistence model, scheduling system, or UI behavior. Detailed semantics are defined in [NEWS_INTELLIGENCE_CONTRACTS.md](./NEWS_INTELLIGENCE_CONTRACTS.md).

FPL-22 extends `@fpl-intelligence/adapters` with central curated news ingestion for project-authored synthetic fixtures and disabled-by-default first-party research inputs. The coordinator enforces the exact compliance allowlist, reviewed policy and environment controls, per-record research rights, bounded reads, runtime kill switches, central cache and deduplication, freshness, provenance persistence, lifecycle propagation, external-processing authorization, and content-free shared usage telemetry. It exposes no live coverage and selects no live source, transport, persistence, scheduler, queue, telemetry provider, or LLM. Operational semantics are defined in [NEWS_INGESTION.md](./NEWS_INGESTION.md).

FPL-28 extends `@fpl-intelligence/database` with versioned PostgreSQL persistence for distinct provider-independent `RawNewsItem`, `Claim`, `Evidence`, `NewsSignal`, and `PlayerAvailabilityState` artifacts. It stores validated content-minimized contracts, explicit lifecycle, expiry, retention, and supersession metadata, and deterministic current/historical availability queries at a caller-supplied UTC time. It preserves conflicting and superseded history, makes retention deletion explicit, and selects no database host, retention scheduler, or provider.

FPL-49 extends `@fpl-intelligence/domain` with the shared provider-independent recommendation contract for lineup, captaincy, and transfer planning. It deterministically ranks a primary option and alternatives; preserves horizons, proposed actions, gross and net impact, hits, assumptions, constraints, risks, confidence factors, structured explanations, evidence conflicts, provenance, lineage, commercial-use state, and material-change triggers; and validates cross-field invariants with typed issues. Recommendations are immutable decision-support snapshots and expose no FPL action execution API. The contract selects no optimizer, persistence model, provider, LLM, or UI behavior. Its semantics and compatibility policy are defined in [RECOMMENDATIONS.md](./RECOMMENDATIONS.md).

FPL-61 extends `@fpl-intelligence/domain` with deterministic personalized-news relevance matching. It evaluates centrally produced, effective NewsSignals against a confirmed TeamState, manually supplied watchlist, recommendation candidate set, and explicit strategic-player configuration. The output records relevance reasons, preserves signal/availability lineage, suppresses duplicates, expired, and unrelated signals, and marks unresolved, unknown, or low-confidence states for review. It introduces no provider fetching, profiles, authentication, per-user collection, or recommendation mutation. Its semantics are defined in [NEWS_RELEVANCE.md](./NEWS_RELEVANCE.md).
